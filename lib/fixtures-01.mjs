...
//                   then recreates it.
//   kb-01-padel     01.8's address, added 2026-09-13 for 8sept-updates.md B3.
//                   A PADEL account, and the only one here that is not football.
//                   Rebuilt every run for the same reason kb-01-wizard is: the
//                   padel rating questionnaire runs once per account. See
//                   THE QUESTIONNAIRE RUNS ONCE below.
...
export const PERSONAL_INFO = {
  ...
  padel: { name: 'Padma', lastName: 'KB', gender: 'Prefer not to say', sports: ['Padel'] },
};

/**
 * THE QUESTIONNAIRE RUNS ONCE, AND THERE IS NO WAY BACK.
 *
 * Measured on staging, 2026-09-13, while writing 01.8:
 *
 *   * the whole nine-screen wizard writes exactly ONE call -
 *     `PUT /users/:userId {rating7, courtPositions, matchType, preferredTime}`;
 *   * once `rating7` is set, the "Complete The Padel Rating Questionnaire"
 *     panel DISAPPEARS from Profile settings, so 01.8's entry point is gone;
 *   * `rating7` cannot be put back. `{"rating7": null}` answers 400 "Expected
 *     number, received null" and `{"rating7": 0}` answers 400 as well.
 *
 * So the account has to be deleted and made again, exactly the way 01.4's
 * wizard account is. `rebuildPadel01()` in lib/kb.ts does it, and 01.8's spec
 * calls it first thing.
 *
 * `/padel-level` stays reachable by URL after the rating is set - it is the
 * same wizard drawn as a full page rather than a modal - but the article
 * documents the entry point a reader can actually find, which is the one that
 * goes away.
 */
export const PADEL_QUESTIONNAIRE_RUNS_ONCE = true;
