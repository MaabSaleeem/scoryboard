...
// --- 05.5, following -------------------------------------------------------
//
// Was collection 06, merged in on 2026-08-31. Following needs things to follow
// that the reader does not own, so ACCOUNTS.mate owns all three of them. Marc
// owns nothing here and follows two of them at rest.
//
// The Follow control is the same control in all three places - the entity's own
// header, beside its counters - and it reads **Follow** or **Unfollow**. That
// sameness is the article's first point, and it is why one fixture shape covers
// a player, a team and a tournament.

/** Teams owned by ACCOUNTS.mate. Marc follows the first and not the second. */
export const FOLLOW_TEAMS = {
  // In the Following window's Teams tab at rest.
  followed: 'KB 05 Rovers',
  // Not followed, so its page shows **Follow** - 05.5's team capture.
  unfollowed: 'KB 05 Wanderers',
};

/**
 * A tournament owned by ACCOUNTS.mate, not followed.
 *
 * A Free account can create one and it is born `status: "Published"`, so no
 * Tournament Pro and no wizard is needed. The dates are fixed, and far enough
 * ahead that nothing about the tournament changes on its own.
 */
export const FOLLOW_TOURNAMENT = 'KB 05 Cup';
export const FOLLOW_TOURNAMENT_DATES = {
  startDate: '2026-11-07T10:00:00.000Z',
  endDate: '2026-11-08T18:00:00.000Z',
};

/** What Marc follows at rest: one player and one team, and nothing else. */
export const FOLLOWS = {
  player: 'mate',                    // a key of ACCOUNTS
  team: FOLLOW_TEAMS.followed,
};

/**
 * The player 05.5 follows and unfollows inside its own run.
 *
 * ACCOUNTS.player is the right one: it is a real account, it is NOT in Marc's
 * friends list, and it is not followed at rest - so the capture of **Follow**
 * is honest and the capture of **Unfollow** is the result of the step before it.
 */
export const FOLLOW_TARGET = 'player';

export const FOLLOW_BUTTON = 'Follow';
export const UNFOLLOW_BUTTON = 'Unfollow';

export const FOLLOWING_DIALOG = {
  // The heading carries the count: "Following (2)".
  title: 'Following',
  // Upper case on screen is CSS - the same trap collection 14 hit on GROUP A -
  // so the tab labels are matched in their original casing, with their counts.
  playersTab: 'Players',
  teamsTab: 'Teams',
  unfollow: 'Unfollow',
};

export const FOLLOWERS_DIALOG = {
  title: 'Followers',
  empty: 'Looks like you don’t have any followers yet.',
};
